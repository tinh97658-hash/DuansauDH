...
describe("development scheduling seed", () => {
  ...
  it("creates the class curriculum chain without pre-creating offerings", () => {
    ...
    expect(select("Curriculum")).toHaveLength(1);
    expect(select("CurriculumBlock")).toHaveLength(1);
    expect(select("CurriculumSubject")).toHaveLength(3);
    // Lớp kế thừa CTĐT; học phần tự chọn do Viện chỉ định cho cả lớp.
    expect(select("ClassGroupElective")).toHaveLength(1);
    expect(select("ClassGroup")[0]).toMatchObject({
      ...schedulingTestScope,
      curriculumId: select("Curriculum")[0].id,
    });
  ...
  it("does not duplicate or overwrite rows on rerun and uses a transaction", async () => {
    ...
    const sequelize = {
      ...
      models: new Proxy({}, { get: () => ({
        findOrCreate: async (options: any) => {
        ...
        update: async (values: any) => {
          saved.set(schedulingTestScope.majorId, saved.get(schedulingTestScope.majorId));
          return [1, values];
        },
      }) }),
    };
